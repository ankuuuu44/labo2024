    ...
    function dispData(msg){
	    ...
	    function getA(req){
            ...
            for (i=0;i<5;i++){
                ...
                mytr.setAttribute('data-key', topFiveItems[i].key); // keyを属性として保存
                ...
                // ダブルクリックイベントを追加
                mytr.addEventListener('dblclick', function (event) {
                    var cells = this.getElementsByTagName('td');
                    var rank = cells[0].innerText;
                    var grammarItem = cells[1].innerText;
                    var correctRate = cells[2].innerText;
                    var key = this.getAttribute('data-key'); // 保存したkeyを取得

                    // コンソールに表示
                    console.log(`選択された行: key=${key}, 順位=${rank}, 文法項目=${grammarItem}, 正解率=${correctRate}`);
            ...
            console.log(response.row_Allquescount["COUNT(*)"]);
            console.log(response.countT_echo);
            var yet_ques = response.row_Allquescount["COUNT(*)"] - response.countT_echo - response.countF_echo;
            console.log("正解数:" + response.countT_echo + "不正解数:" + response.countF_echo + "未回答数:" + yet_ques);
            console.log(yet_ques);

            var chart_ques_ratio = new CanvasJS.Chart("solve-ratio",
            {
                title:{
                    text: "解答割合"
                },
                ...
                data: [
                {
                    ...
                    dataPoints: [
                        { y: response.countT_echo, indexLabel: "正解"},
                        { y: response.countF_echo, indexLabel: "不正解" },
                        { y: yet_ques, indexLabel: "未回答" }
                    ]
                }],
                
            });

            chart_ques_ratio.render();
...
<div class="count">
    ...
    <button type = "button" id = "machineLearningbutton">機械学習</button>
</div>
<DIV class="search">
    <div align="left">
        <?php
                        ...
                        <tr>
                            ...
                            <td>
                                <ul class="itemgroup">
                                    ...
                                    <li><label for="groupingDDcountbool"><input type = "checkbox" name = "featureLabel[]" value = "groupingCountbool">グループ化回数</label></li>
                                </ul>
                            </td>
                        </tr>
                        <tr>
