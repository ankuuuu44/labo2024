...
function jump(){
    location.href = "http://lmo.cs.inf.shizuoka.ac.jp/~miki/create/point_check.php";
}
...
<script type="text/javascript">
    ...
    var delwid;
    var nextflag = 0; //終了の時にラベルを動かさないようにするため
    ...
    var region = 0;
    var URL = 'http://lmo.cs.inf.shizuoka.ac.jp/~miki/mondai/' //サーバー用
    var URL_sub = 'http://lmo.cs.inf.shizuoka.ac.jp/~miki/create/' //サーバー用
    //ランダムに配列を並び替えるソース
    ...
    parameters: $params
                ...
                function getResponse(req) {
                    ...
                    $q = "start";
                    $params = 'param1=' + encodeURIComponent($q);
            ...
            parameters: $params
                    ...
                    function getStart(req1) {
                        ...
                        req1.responseText = req1.responseText.substr(1);// 13/03/03臨時対処　先頭に変な文字が入るので消す
                        /*
                        var str = new String();
                        str = req1.responseText;
                        var b = req1.responseText.length;
                        alert(b);
                        var n = str.substr(1, 1);
                        alert(n);
                        */
                        /*
                        var n = str.substr(5, 10);
                        alert(n);
                        }
                        var n = str.substr(10, 15);
                        alert(n);
                        }*/
                        /*
                        var n = str.substr(16, 17);
                        alert(n);
                        */
                        //alert(req1.responseText.length);
                    ...
                    function getFix(Fix) //固定情報の表示
                    {
                        ...
                        //alert(StartQues.length);

                        //Mylabels配列のコピー。Mylabelは今後動かさないので。
                        Mylabels2 = Mylabels.concat();
                        //-------------------------------------
                        var $j = "japanese";
                        $params = 'param1=' + encodeURIComponent($j);
                        new Ajax.Request(URL_sub + 'syori.php',
                            {
                                method: 'get',
    ...
    function MyLabels_MouseDown(sender) {
        if (nextflag == 1) { alert("問題終了後は並べ替えは行うことができません"); return; }
    ...
    function Button1_Click() {
        ...
        /*
        var fix_a = confirm("初期順序が正答文と非常に近いですかよろしいですか？");
        if (fix_a == true) {
        continue;
        }
        else {
        ...
        */
        ...
        for (i = 0; i <= Mylabels2.length - 1; i++) {
            ...
            if (Mylabels_ea[i].innerHTML == "/") {
                ...
                if (i < Mylabels2.length - 1) {
                    MyAnswer += Mylabels_ea[i].innerHTML + " ";
                } else if (i == Mylabels2.length - 1) {
                    MyAnswer += Mylabels_ea[i].innerHTML;
                } else {
                    ;
                }
            }
            //alert(Mylabels_ea[i].innerHTML);
            //alert(Mylabels_ea[i].innerHTML.length);
            //alert(MyAnswer);
        }
        MyAnswer += "."; //とりあえず仮
        ...
        $q = "another";
        //alert($MyAnswer);
        //$params = "param1='" + encodeURIComponent($MyAnswer) + "'";
        $params = 'param1=' + encodeURIComponent($MyAnswer)
        + '&param2=' + encodeURIComponent($part_point)
        + '&param3=' + encodeURIComponent($q);
        //alert($params);
        new Ajax.Request(URL_sub + 'connect.php',
        {
            method: 'get',
            onComplete: getA,
            parameters: $params
    ...
    function Button3_Click() {
        ...
        YAHOO.util.Dom.setStyle("Button1", "display", "block");
        //YAHOO.util.Dom.setStyle("PointLabel", "display", "block");
        //YAHOO.util.Dom.setStyle("PointSelect", "display", "block");
    }
    //正誤表示
...
<input type = "button"
	...
	style="width:80px;height:36px;position:absolute;left:768px;top:80px"/>
...
<input type = "button"
	...
	style="width:80px;height:36px;position:absolute;left:768px;top:80px;display:none"/>
...
<!--自信度--> 
<label for="PointSelect" id="PointLabel" style="position:absolute;left:785px;top:30px;display:none">
得点</label>
<select 
	id = "PointSelect"
	size="1"
	onChange="QuesLevelChange()"
	style="position:absolute;left:790px;top:45px;display:none">
