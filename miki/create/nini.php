...
<script type="text/javascript">
    ...
    var delwid;
    var URL = 'http://lmo.cs.inf.shizuoka.ac.jp/~miki/create/' //サーバー用
    //var URL = 'http://localhost/mondai/' //ローカル用
        ...
        function getaca(res) {
            ...
            function getara(res) {
                ...
                if (AllResAns != 0) {
                    ...
                    $AllPer = AllCorrectAns / AllResAns;
                } else {
                    AllCorrectAnsRate = 0;
                    $AllPer = 0;
                }
                ...
                function getrankb(res) {
                    //alert(res.responseText); //戻り値確認用
                    res.responseText -= 0;
                    ResRank = res.responseText;
                    //alert($AllPer);
                    if (AllCorrectAnsRate == 0) {
                        CorrectRank = ResRank;
                        ...
                        $rank = "s";
                        $params = 'param1=' + encodeURIComponent($rank)
                                    + '&param2=' + encodeURIComponent($AllPer);
                        //alert($AllPer);
                        new Ajax.Request(URL + 'rank.php',
                            {
                                method: 'get',
                        ...
                        function getranks(res) {
                            //alert(res.responseText); //戻り値確認用
                            res.responseText -= 0;
                            CorrectRank = res.responseText + 1;
                            //順位---------
                            if (CorrectRank == -1) {//正解率0％は最下位
                                CorrectRank = ResRank;
                            }
                            AllCorrectAnsRate = Math.round(AllCorrectAnsRate * 100) / 100; //小数第三位を四捨五入するため
                            document.getElementById("ListBox1").innerHTML = "全体 ( " + AllCorrectAns + " / " + AllResAns + "  " + AllCorrectAnsRate + "% )</br>"
    ...
    */
    ...
    function ButtonM_Click() {
        ...
        //alert(AID);
        if (AID % 5 == 0 && AID < 30) {
            alert("一度書き込みを行います。右下の終了ボタンを押してください。");
            document.getElementById("Button2").disabled = true;
        } else if (AID == 30) {
            alert("実験終了です。ご協力ありがとうございました。\n右下の終了ボタンを押して終了してください。");
            document.getElementById("Button2").disabled = true;
        }
    }
    //----------------------------------------
    ...
    function TermTextChange() {
        ...
        index = obj.selectedIndex;
        if ((index != 0) && (document.Questions.TermText.value != "")) {
            DocStrL.disabled = false;
            YAHOO.util.Dom.setStyle("Button2", "display", "block");
            DocStr2.disabled = false;
        } else {
...
<input type = "button"
	...
	style="width:80px;height:30px;position:absolute;left:600px;top:220px;display:none"/>
<!--次の問題ボタン-->
<input type = "button"
	id = "Button2"
	value="次の問題"
...
<div id = "RichTextBox3" style="background-color:#a1ffa1;position:absolute;
	 left:668;top:422;width:80;height:34;border-style:inset;display:none">正誤を表示</div>
<!--解答時間-->
...
<form name="Questions">
<!--アンケートの確信度--> 
<label for="QuesLevel" id="QuesLabel" style="position:absolute;left:600px;top:170px;display:none">
解答の自信度</label>
<select 
	id = "QuesLevel"
	size="1"
	onChange="QuesLevelChange()"
	style="position:absolute;left:600px;top:190px;display:none">
<option value="choose" selected="selected">選択してください</option>
<!--<option value="level5">5:自信があった</option>-->
...
<!--
<label for="TermText"
		id="TermLabel" style="position:absolute;left:600px;top:230px;display:none">迷いの生じた単語</label>
<input type="text" onkeyup="TermTextChange()" onChange="TermTextChange()" id="TermText" style="position:absolute;
	 left:600;top:250;font-size:12;display:none"/>
	 -->
<!--ダミー-->
