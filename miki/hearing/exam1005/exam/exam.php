...
var Hushflag = new Boolean(false);
var URL = 'http://lmo.cs.inf.shizuoka.ac.jp/~sato/test/exam/' //サーバー用
	...
	new Ajax.Request(URL + 'swrite.php',
{
	...
	//ドラッグ開始地点の保存
	function getA(req){
	alert(req.responseText + "\nここでは、あなたのマウスの挙動を検査します。\nランダムに表示された数字をドラッグ＆ドロップで小さい順に並べてください。\n回数は１０回です。");
	}
	function getE(req){
		alert("書き込みに失敗しました");
	}
...
new Ajax.Request(URL + 'correct.php',
{
	...
	function getaca(res){
		...
		new Ajax.Request(URL + 'correct.php',
		//new Ajax.Request('http://localhost/test/correct.php',
		{
			method: 'get',
			onSuccess: getara,
			onFailure: getError,
...
function setques(){
	Fixmsg.innerHTML = "-情報-";
	myCheck(0);
	//$Ques_Num = Math.floor( Math.random() * 200 ); //ランダムに問題を選出
	$Ques_Num = Count; //問題を指定したいとき用
	Count += 1;
	$q = "q";
	//alert($Ques_Num);
...
function getResponse(req)
{
	...
	//-------------------------------------条件抽出
			var $term = "term";
	$params = 'param1=' + encodeURIComponent($Ques_Num)
				+ '&param2=' + encodeURIComponent($term);
...
<div id = "RichTextBox1" style="background-color:#cccccc;position:absolute;
     left:12;top:27;width:731;height:36;border-style:inset;display:block">
                                   	   指定された条件に合うように数字を並べ替えてください（１０問解くまで終了しないでください）</div>
<!--回答
