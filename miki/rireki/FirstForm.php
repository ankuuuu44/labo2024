                ...
                if($row_track["count(*)"] ==1){
                    ...
                    while ($row_mouse = mysql_fetch_array($res_mouse)){//マウス軌跡情報がある限りループ
                        ...
                        if($row_mouse["DD"]==2){//Dragした時
                            if($DStart_Flag == 0){
                                ...
                                if($row_mouse["Y"]<= 130){ $Array_flag =0; }//問題提示欄
                                else if($row_mouse["Y"] > 130 and $row_mouse["Y"]<= 215){
                                    $Array_flag =4; 
                                    }//解答欄
                                else if($row_mouse["Y"] > 215 and $row_mouse["Y"]<=295){
                                    $Array_flag =1; 
                                    }//レジスタ1
                                else if($row_mouse["Y"] > 295 and $row_mouse["Y"]<=375){
                                    $Array_flag =2; 
                                    }//レジスタ2
                                else if($row_mouse["Y"] > 375){
                                    $Array_flag =3; 
                                    }//レジスタ3
                                }
                            }
                        ...
                        if($row_mouse["DD"]==1){//Dropした時
                            ...
                            //echo "DDした";
                            if($row_mouse["Y"]<= 130){//問題提示欄
                                if($Array_flag ==0){$DD_QQ_Count += $label_count; 
                                    $QQFlag = 1;//問題提示欄内起き直し動作なので、DDに含まない
                                    if($GP ==1){ 
                                        $GroupCount--;//QQはDD回数に含まない
                                    }
                                }else if($Array_flag ==4){$DD_AQ_Count += $label_count; //解答欄→問題提示欄
                                    $DragDropCount_rev += $label_count;
                                    if($GP ==1){ $AQ++;}
                                }else{$DD_RQ_Count += $label_count;//レジスタ→問題提示欄
                                    $DragDropCount_rev += $label_count;
                                    if($GP ==1){ $RQ++;}
                                }
                              }else if($row_mouse["Y"] > 130 and $row_mouse["Y"]<= 215){//解答欄  
                                if($Array_flag ==0){$DD_QA_Count += $label_count; //問題提示欄→解答欄
                                    ...
                                    $DragDropCount_rev += $label_count;
                                    if($GP ==1){ $AA++;}
                                }else{$DD_RA_Count += $label_count; //レジスタ→解答欄
                                    if($GP ==1){ $RA++;}
                                    $sub_UY +=2;
                                }
                            }else if($row_mouse["Y"] > 215){   
                                if($Array_flag ==0){$DD_QR_Count += $label_count; //問題提示欄→レジスタ
                                    ...
                                    if($GP ==1){ $RR++;} 
                                    if($row_mouse["Y"] > 295 and $row_mouse["Y"]<=375){ //レジスタ2内入れ替え
                                        $DD_RR_Count += $label_count; 
                                        $DragDropCount_rev += $label_count;
                                    }else{ //レジスタ2→他のレジスタ
                                        $DD_RR_rev += $label_count; 
                    ...
                    */
                    ...
                    $z=0;
                        ...
                        $sql_UTurndef = "update linedatamouse set UTurnX = 0,UTurnY=0  where UID= ".$UID_reserve[$i]. " and AID = ".$AID_reserve[$i];
                        $res_UTurndef = mysql_query($sql_UTurndef,$conn);
                    ...
                    */
                    ...
                    $DD_AA_Count,$DD_AR_Count,$DD_RQ_Count,$DD_RA_Count,$DD_RR_Count,$DD_RR_rev,$UTurnCount_X,$UTurnCount_Y,$UTurnCount_XinDD
                    ,$UTurnCount_YinDD,0)";
