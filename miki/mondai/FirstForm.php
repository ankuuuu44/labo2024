                    ...
                    if($mouse_count !=0){
                       ...
                       $X_change = $row_mouse["X"] - $before_X;  //Xの変化量
                        ...
                        if($UTurnFlag_X ==0){//初回
                            ...
                            $X_point = $row_mouse["X"];
                            $X_time = $row_mose["Time"];
                        }else if($UTurnFlag_X ==1){
                            if($X_change> -1*($border) and $X_change<0){//-(基準値)<変化量<0
                                //echo "X維持";
                                
                                $move_distanceX +=  $X_change;
                                echo "変化量".$move_distanceX;
                                if($move_distanceX < -1 * $border){//変化量合計が負方向15を超えたらUターン
                                    $move_distanceX = 0;
                                    $UTurnFlag_X = -1;
                                    $UTurn_i[$UTurnCount_X + $UTurnCount_XinDD] = $X_point;
                                    echo "Uターン座標".$X_point;
                                    if($DD_flag == 0){  
                                    echo "刻みUターンX(+⇒-)";
                            ...
                            }else if($X_change> 0 and $move_distanceX < 0){//変化が正方向でここまでの負方向の変化量より多くなったら
                                   $move_distanceX +=  $X_change;
                                    echo "変化量".$move_distanceX;
                                    if($move_distanceX >0){
                                        $move_distanceX = 0;
                                        $X_point = $row_mouse["X"];//Uターン判定に使うX座標を現在の座標に変更 
                                        $X_time = $row_mose["Time"];                            
                                    }
                            }else if($X_change <=(-1)*$border){//変化量が負方向15pixel以上ならば
                            ...
                            if($X_change< $border and $X_change>0){//変化量が正方向のに基準値以下ならば
                                //echo "X維持";
                            ...
                            }else if($X_change< 0 and $move_distanceX > 0){//変化が負方向でここまでの正方向の変化量より多くなったら
                            ...
                            }else if($X_change >=$border){//変化量が正方向15pixel以上ならば
                        ...
                        if($UTurnFlag_Y ==0){
                            ...
                            if($Y_change> -1*($border) and $Y_change<0){//-(基準値)<変化量<0
                                //echo "X維持";
                                
                                $move_distanceY = $move_distanceY +  $Y_change;
                                //echo $move_distanceY;
                                if($move_distanceY < -1 * $border){//変化量合計が(-border)を超えたらUターン
                                    $move_distanceY = 0;
                                    $UTurnFlag_Y = -1;
                                    if($DD_flag == 0){  
                            ...
                            }else if($Y_change> 0 and $move_distanceY < 0){//変化が正方向でここまでの負方向の変化量より多くなったら
                                   $move_distanceY = $move_distanceY +  $Y_change;
                                    //echo $move_distanceY;
                                    if($move_distanceY >0){
                                        $move_distanceY = 0;                             
                                    }
                            }else if($Y_change <=(-1)*$border){//変化量が-border以上ならば
                            ...
                            if($Y_change< $border and $Y_change>0){//変化量が正方向のに基準値以下ならば
                                //echo "Y維持";
                                
                                $move_distanceY = $move_distanceY +  $Y_change;
                                //echo $move_distanceY;
                                if($move_distanceY > $border ){//変化量合計がborderを超えたらUターン
                                    $move_distanceY = 0;
                                    $UTurnFlag_Y = 1;
                            ...
                            }else if($Y_change< 0 and $move_distanceY > 0){//変化が負方向でここまでの正方向の変化量より多くなったら
                            ...
                            }else if($Y_change >=$border){//変化量がborder以上ならば
