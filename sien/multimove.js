...
function fetchStudentslist() {
    fetch('../fetch_students_list.php')
        ...
        .then(data => {
            ...
            const selectElement = document.getElementById('multi-learner-list');
...
function displayLearnerDetails(selectedUID) {
    const studentname = document.getElementById('multi-student-name');
    
    // 現在表示されている学習者名を取得
    let currentNames = studentname.innerHTML;

    // すでに選択された学習者が表示されているかチェック
    if (!currentNames.includes(selectedUID)) {
        // 現在の内容に新しく選択された学習者を追加
        studentname.innerHTML += `<span class="text-highlight">${selectedUID}</span> `;
    }
}
