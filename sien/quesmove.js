...
function fetchQuesinfoAll(selectedWID) {
    fetch('../fetch_ques_info_all.php?wid=' + selectedWID)
    .then(response => response.json())
    .then(data => {
        const quesinfo = data.quesinfo;
        console.log("aaaここに来てるよ");
        console.log(quesinfo);
        forEach(quesinfo, (quesinfo) => {
            relativeSentence(quesinfo.Sentence, quesinfo.EndSentence);
        })
        /*
        //const questionname = document.getElementById('ques-name');
        displayQuesSentence(quesinfo);
